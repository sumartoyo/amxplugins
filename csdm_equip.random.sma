...
new g_NextSecWeapons[33][18]
new g_PrimWeapons[33][18]
new g_NextPrimWeapons[33][18]
...
public plugin_init()
{
	...
	//register_clcmd("say rr", "rehealth")
...
public csdm_PostDeath(killer, victim, headshot, const weapon[])
{
	if (killer && killer != victim) {
		...
		static next_kill
		next_kill = g_KillTarget - g_KillCount[killer]

		if (!is_user_bot(killer)) {
            if (next_kill <= 0) {
				// client_print(killer, print_chat, "You've got ^x04%s",
				client_print_color(killer, -1, "^1You've got ^3%s",
					strlen(g_NextPrimWeapons[killer]) > 0 ? g_NextPrimWeapons[killer] : g_NextSecWeapons[killer]
				);
			} else {
				// client_print(killer, print_chat, "^x04%i ^x01more %s to ^x04%s",
				client_print_color(killer, -1, "^3%i ^1more %s to ^3%s",
					next_kill,
					next_kill == 1 ? "frag" : "frags",
					strlen(g_NextPrimWeapons[killer]) > 0 ? g_NextPrimWeapons[killer] : g_NextSecWeapons[killer]
				);
			}
	    }
...
equipUser(id, to)
{
	...
	if (g_KillCount[id] >= g_KillTarget) {
		g_KillCount[id] = 0
		GiveUserNextWeapon(id)
	} else {
		if (strlen(g_SecWeapons[id]) == 0 && strlen(g_PrimWeapons[id]) == 0) {
			g_KillCount[id] = 0
			GenerateRandomWeapon(id)
			GiveUserNextWeapon(id)

			if (!is_user_bot(id)) {
				switch (team) {
					case _TEAM_T: {
						//client_print(id, print_chat, "Playing as ^x03Terrorist")
						client_print_color(id, id, "^1Playing as ^3Terrorist")
					}
					case _TEAM_CT: {
						//client_print(id, print_chat, "Playing as ^x03Counter-Terrorist")
						client_print_color(id, id, "^1Playing as ^3Counter-Terrorist")
					}
				}
			}
		} else {
			GiveUserCurrentWeapon(id)
		}
	}
...
GiveUserCurrentWeapon(id)
{
	...
	if (strlen(g_PrimWeapons[id]) > 0)
		GiveUserFullWeapon(id, g_PrimWeapons[id])
}

GiveUserNextWeapon(id)
{
	GiveUserFullWeapon(id, g_NextSecWeapons[id])
	copy(g_SecWeapons[id], 17, g_NextSecWeapons[id])

	if (strlen(g_NextPrimWeapons[id]) > 0)
		GiveUserFullWeapon(id, g_NextPrimWeapons[id])
	copy(g_PrimWeapons[id], 17, g_NextPrimWeapons[id])
